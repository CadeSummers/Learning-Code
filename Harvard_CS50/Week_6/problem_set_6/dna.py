...
def dnacheck(dnacount, check, row, count):
    ...
    #print(f"Comparing {dnacount[check - 1]} to {row[check - 1]}")
    #print(f"comparing {dnacount} at element {check}: ({dnacount[check]}) to {row} at element {check}: ({row[check]})")  
    ...
    if dnacount[check] == int(row[check]):
        ...
        #print(f"check reduced from {check} to {check - 1}")
        #reduce check by 1
        check-=1
        ...
        #recursive statement if check is greater than 0 recall this function
            ...
            dnacheck(dnacount, check, row, count)
...
for row in csvfile:
    ...
    if dnacheck(dnacount, check, row, count):
        print(f"{names[count]}")
        exit(0)
